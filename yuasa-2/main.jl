...
for id in ids 
    plot_rc_param_trajectory(kfs[id], sols[id]) |> display
end
Ω
...
param_modules = Dict(id =>
    Dict(
        :Q => calc_Q(kfs2[id], sols2[id], f.ocv⁻¹; v=vlim, n=12),
        :soc => calc_soc0(kfs2[id], sols2[id], f.ocv⁻¹; v=vlim, n=12),
        :soh => calc_soh(kfs2[id], sols2[id], f.ocv⁻¹, 100; v=vlim, n=12),
    ) for id in ids2
)
...
let id = (; p=3, m=5, c=1)
    plot_q_estimation(kfs[id], sols[id])
end


# TODO: check that for all cells, arrhenius params are positive