...
- Predict step:
  $$
  \begin{equation}
      \hat{x}_{k | k-1} = A \cdot \hat{x}_{k -1|k -1} + w_{k}
  \end{equation}
  $$
...
$$
\begin{equation}
  g^b_0 \sim \mathcal{N}(\hat{g}^b_0, P^b_0),\quad \hat{g}^b_0 =m(X_b)
\end{equation}
$$
...
From the derivation above one can infere the prediction step. On which $g^{t}_{k-1}$ is the equivalent of $x_{k | k-1}$ of eq.(4)
...
Now, once the problem has been set up as a Kalman filter step, it can be easly following eq.5.
For the system one can consider 2 cases:
...
- Finally, one eq. (35) one can observe that $\hat{g}^{t}_{k - 1} = H_t \hat{g}^{b}_{k - 1} - \hat{b}_t$, thus:
  $$
  \begin{equation}
