function plot_profile2(batt, l_ocv, l_r, σ_f1, σ_f2, Vrc, soc_test; dt=nothing, normalize=false)
    ##PLot
    limit_predict = [0, 1]
    step_predict = 0.01
    ...
    if Vrc == false
        V_p = RecursiveGPs.predict(rgp_ocv, soc_test[1:50:end], train=false).μ +
              i_test[1:50:end] .* RecursiveGPs.predict(rgp_r, soc_test[1:50:end], train=false).μ
    ...
    #Voltage_profile
    ax3 = Axis(fig[3, 1], title="Voltage profile")
    lines!(ax3, V_r, label="Real")
    lines!(ax3, V_p, label="Aprox")

    #Error
    ax4 = Axis(fig[4, 1], title="Absolute Error")
