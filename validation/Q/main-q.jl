...
θ = (; # hyperparams
    ...
    # q=(; σ1=0.5, σ2=1e-6, q0=5.0),
    q=(; σ1=0.5^2, σ2=1e-8, q0=5.0),
)
ϑ = (; Ts=10.0)
# ϑ = (; Ts=1.0, q=5.0)
kf = build_kf(θ, ϑ, focv´, zt; n=21);

@time sol = LLPF.forward_trajectory(kf, us, ys);
...
sol2 = run_sim!(kf, us, ys, ut)

plot_ecm(kf, df, zt, focv, fR0; closeup=true)

calc_ocv_mae(kf, df_train, zt, focv)

plot_sim(sol2, df, ttrain)

# ===
# wrong initial OCV
soc0 = df_train[begin, :s]
soc0´ = focv3⁻¹(df_train[begin, :v])
θ = (; # hyperparams
    ocv=(; σ=0.05, ℓ=0.9),
    r0=(; σ=0.001, ℓ=0.5),
    # soc=(; σ1=0.01, σ2=1e-7, soc0=0.65),
    soc=(; σ1=0.01, σ2=5e-8, soc0=soc0´),
    # q=(; σ1=0.5, σ2=1e-6, q0=5.0),
    q=(; σ1=0.5^2, σ2=5e-8, q0=5.0),
)
ϑ = (; Ts=10.0)
kf2 = build_kf(θ, ϑ, focv3´, zt; n=21)

@time sol = LLPF.forward_trajectory(kf2, us, ys);

# plot_ecm(kf2, df, zt, focv, fR0; focv_prior=focv3´)
plot_ecm(kf2, df, zt, focv, fR0; focv_prior=focv3´, closeup=true)

sol2 = run_sim!(kf2, us, ys, ut)

Δs = soc0 - focv3⁻¹(focv(soc0))
plot_ecm(kf2, df, zt, focv, fR0; focv_prior=focv3´, soc_shift=0.0, closeup=true)
plot_ecm(kf2, df, zt, focv, fR0; focv_prior=focv2´, soc_shift=Δs, closeup=true)
plot_ecm(kf2, df, zt, focv, fR0; focv_prior=focv2´, soc_shift=-0.03, closeup=true)
...
θ = (; # hyperparams
    ocv=(; σ=0.1, ℓ=0.1),
    r0=(; σ=0.001, ℓ=0.5),
    soc=(; σ1=0.02, σ2=1e-7, soc0=soc0),
)
ϑ = (; Ts=10.0, q=4.8)
kf2 = build_kf(θ, ϑ, focv´, zt; n=21)

plot_ecm(kf2, df, zt, focv, fR0)

@time sol2 = run_sim!(kf2, us, ys, ut)

# Δs = soc0 - focv2⁻¹(focv(soc0))
plot_ecm(kf2, df, zt, focv, fR0; soc_shift=0.0, closeup=true)
# plot_ecm(kf2, df, zt, focv, fR0; soc_shift=Δs, closeup=true)

calc_ocv_mae(kf2, df_train, zt, focv)
