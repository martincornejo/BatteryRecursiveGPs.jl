...
function dynamics!(x⁺, x, u, p, t)
    ...
    x⁺ .= x # previous values
    x⁺.rc .= dynamics_rc(x⁺.rc,u,p.rc,t)
...
function measurement(x, u, p, t)
    ...
    rc = measurement_rc(xc.rc, u, p.rc, t)
    ocv + u.î * r0 + rc |> SVector{1}
end
...
function R2(x, u, p, t)
    ...
    rc = R2fun_rc(xc.rc,u,p.rc,t)
    ocv + u.î^2 * r0 + + rc + vσ |> SMatrix{1,1}
end
...
function build_kf(θ, ϑ, focv_prior, zt; n=21)
    ...
    τ0 = 100
    R0 = 7e-3
    rc = RC(ts = θ.rc.ts,τ0 = θ.rc.τ0,R0 =θ.rc.R0)
    ...
    rgps = (; soc, ocv=rgp1, r0=rgp2, rc = rc)
