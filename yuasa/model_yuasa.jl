...
function dynamics_rc(x, i, p)
    (; Ts) = p
    (; v, r, τ) = x
    exp(-Ts / exp(τ)) * v + i * exp(r) * (1 - exp(-Ts / exp(τ)))
end
...
function dynamics!(x⁺, x⁻, u, p, t)
    ...
    xc⁺.rc.v = dynamics_rc(xc⁻.rc, u.i[1], p)
    nothing # IPD
end
...
function measurement(x, u, p, t)
    ...
    vrc = xc.rc.v # measurement r
    ocv .+ u.i[1] .* r0 .+ vrc |> SVector{1}
end
...
function model_predict(kf, u, x = kf.x)
    (; xid, vσ²) = kf.p
