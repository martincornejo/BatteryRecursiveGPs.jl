...
function measurement(x, u, p, t)
    (; build_gp, xid, b0, μ0) = p
    # (; g) = x
    # (; b) = u
    (; g, θ) = ComponentVector(x, xid)
    b = u
    gp = build_gp(θ)
    Σ0⁻¹ = inv(cov(gp, b0) + 1e-6I)
...
# kf = UnscentedKalmanFilter(dynamics, measurement, R1, R2fun, d0; nx=length(x), ny=1, nu=1, p)
kf = ExtendedKalmanFilter(dynamics, measurement, R1, R2fun, d0; nx=length(x), ny=1, nu=1, p)
